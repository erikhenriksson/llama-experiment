...
from llama.dataset import DataLoader
import inspect
from torch.optim import Optimizer
from llama.metrics import hellaswag_evaluation
...
class Trainer:
        ...
        for epoch in range(self.n_epochs):
            ...
            for iter in range(self.max_iters):
                ...
                for micro_iter in range(self.grad_accum_iters):
                    ...
                    if self.use_ddp:
                        self.model.require_backward_grad_sync = (
                            micro_iter == self.grad_accum_iters - 1
                        )
                    with torch.autocast(
                ...
                if iter % 250 == 0 or is_last_step:
                    ...
                    idx = self.tokenizer.tokenize_str("Once upon a time")
                    idx = idx.to(self.device)
                    generated = self.raw_model.generate(idx, 25)
                    gen_str = self.tokenizer.decode_tokens(generated)
                ...
                tokens_processed = (
                    self.train_loader.cfg.n_batches
                    * self.train_loader.cfg.n_tokens
                ...
                if self.main_process:
                    if self.monitor:
                        print(
                            f"Epoch {epoch} | iter {iter:5d} | loss: {accumulated_loss.item():.6f} | lr {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | tok/sec: {tokens_per_sec:.2f}"
                        )
                    with open(self.log_file, "a") as f:
                        f.write(
                            f"Epoch {epoch} | iter {iter} | train loss: {accumulated_loss.item():.6f}\n"
                        )
    ...
    def validate(self, epoch, iter, is_last_iter) -> None:
        ...
        if self.main_process:
            ...
            with open(self.log_file, "a") as f:
                f.write(
                    f"Epoch {epoch} | iter {iter} | validation loss: {val_loss_accum.item():.3f}\n"
                )
            if iter > 0 and (iter % 5000 == 0 or is_last_iter):
                checkpoint_path = os.path.join(
                    self.log_dir, f"model_epoch{epoch}_iter{iter:05d}.pt"
                )
                checkpoint = {
    ...
    def evaluate(self, epoch, iter) -> None:
        ...
        if "Hellaswag" in self.metrics:
            num_correct_norm, num_total = hellaswag_evaluation(
                self.model,
                self.ddp_world_size,
                self.ddp_rank,
                self.device,
            ...
            if self.use_ddp:
                num_total = torch.tensor(
                    num_total, dtype=torch.long, device=self.device
                )
            ...
            if self.main_process:
                ...
                with open(self.log_file, "a") as f:
                    f.write(
                        f"Epoch {epoch} | iter {iter} | Hellaswag accuracy: {acc_norm:.3f}\n"
                    )
